...
app
  ...
  .post("/upload", upload.single('file'), (req, res) => {
    ...
    res.send("https://himaval.xyz/i/"+name);
  })
  .get("/i/:filename", (req, res) => {
    const filename = req.params.filename
    const text = fs.readFileSync("file.html", "utf-8").replaceAll("$imgurl", "https://himaval.xyz/" + filename).replace("$title", filename)
    res.send(text)
  })
;
